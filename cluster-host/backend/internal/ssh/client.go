...
func LoadServerSSHInfo(db *sql.DB, encKey string, serverID string) (ServerSSHInfo, error) {
	var user, encKeyStr, encPassStr, host, osFamily string
	var port int
	err := db.QueryRow(
		...
		serverID,
	).Scan(&user, &encKeyStr, &encPassStr, &host, &port, &osFamily)
	if err != nil {
		return ServerSSHInfo{}, err
	}
	decKey, _ := crypto.Decrypt(encKeyStr, encKey)
	decPass, _ := crypto.Decrypt(encPassStr, encKey)
	return ServerSSHInfo{ServerID: serverID, User: user, Key: decKey, Password: decPass, Host: host, Port: port, OSFamily: osFamily}, nil
}
...
func RunSSHCommand(serverID, user, password, key, host string, port int, command string) (string, error) {
	...
	if serverID != "" && serverID != "test" {
		if _, online := websocket.Manager.Get(serverID); online {
			...
			respBytes, err := websocket.Manager.SendCommand(serverID, "POST", "exec-command", reqBody)
			if err != nil {
				return "", err
			}
			var resp struct {
...
func SSHInstallAgent(db *sql.DB, info ServerSSHInfo, serverID string, endpoint string) error {
